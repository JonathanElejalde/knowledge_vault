...
def upgrade() -> None:
    ...
    op.drop_constraint('learning_projects_category_id_fkey', 'learning_projects', type_='foreignkey')
    ...
    op.create_table('categories',
        ...
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=True),  # This should be nullable
    ...
    op.create_foreign_key('learning_projects_category_id_fkey', 'learning_projects', 'categories', ['category_id'], ['id'])
...
def downgrade() -> None:
    ...
    op.create_table('categories',
        ...
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),  # This was the problematic constraint
        sa.Column('name', sa.String(length=100), nullable=False),
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('meta_data', sa.JSON(), nullable=False),
        sa.PrimaryKeyConstraint('id')
