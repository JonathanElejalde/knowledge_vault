        ...
        for note_id, embedding in vectors:
            try:
                ...
                if note:
                    ...
                    logger.warning(f"Note with ID {note_id} not found for vector upsert")
        ...
        try:
            ...
            if filters:
                ...
                if "learning_project_id" in filters:
                    filter_conditions.append("AND n.learning_project_id = :learning_project_id")
            ...
            for row in rows:
                results.append({
                    "id": str(row.id),
                    "score": float(1.0 - row.similarity_distance),  # Convert distance to similarity score
                    "metadata": {
                        "title": row.title,
                        "content": row.content,
                        "tags": row.tags or [],
                        "user_id": str(row.user_id) if row.user_id else None,
                        "learning_project_id": str(row.learning_project_id) if row.learning_project_id else None,
