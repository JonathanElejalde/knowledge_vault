...
async def update_pomodoro_preferences(
    ...
    user = await crud.update_user_preferences(
        db=db, user_id=current_user.id, preferences=preferences_dict
    )
    if not user:
...
async def complete_session(
    ...
    session = await crud.complete_session(
        db=db, session_id=session_id, user_id=current_user.id, session_in=session_in
    )
    if not session:
        raise HTTPException(
...
async def list_sessions(
    ...
    status_filter: Optional[str] = Query(
        None, alias="status", pattern="^(in_progress|completed|abandoned)$"
    ),
    ...
    for session_db in sessions_db:
        ...
        response_list.append(
            SessionResponseWithProject(
                **session_data, learning_project=project_response_data
            )
        )
...
async def get_weekly_statistics(
    ...
    """
    return await crud.get_weekly_statistics(db=db, user_id=current_user.id)
...
async def get_session_summaries(
    ...
    db: Annotated[AsyncSession, Depends(get_db)],
    period: str = Query(
        "week", pattern="^(week|month)$", description="Time period to summarize"
    ),
    start_date: Optional[datetime] = Query(
        None, description="Start date for custom date range (ISO 8601)"
    ),
    end_date: Optional[datetime] = Query(
        None, description="End date for custom date range (ISO 8601)"
    ),
    limit: int = Query(
        10, ge=1, le=100, description="Maximum number of project summaries to return"
    ),
