...
async def get_dashboard_stats(
    ...
    focus_time_result = await db.execute(
        select(func.coalesce(func.sum(Session.actual_duration), 0)).where(
            and_(*session_conditions)
        )
    )
    ...
    notes_count_result = await db.execute(
        select(func.count(Note.id)).where(and_(*note_conditions))
    )
    notes_created = notes_count_result.scalar() or 0

    # Get active projects count
    active_projects_result = await db.execute(
        select(func.count(LearningProject.id)).where(
            and_(
...
async def get_project_stats(
    ...
    sessions_subq = (
        select(
            Session.learning_project_id, func.count(Session.id).label("sessions_count")
        )
    ...
    notes_subq = (
        select(Note.learning_project_id, func.count(Note.id).label("notes_count"))
    ...
    query = (
        ...
        .outerjoin(
            sessions_subq, LearningProject.id == sessions_subq.c.learning_project_id
        )
    ...
    return [
        ...
        or row.notes_count > 0  # Only include projects with activity
    ]
...
async def _get_earliest_activity_date(
    ...
    earliest_session_query = select(func.min(func.date(timezone_expr))).where(
        Session.user_id == user_id
    )

    # Get earliest note date
    earliest_note_query = select(func.min(func.date(timezone_expr))).where(
...
async def get_daily_activity(
    ...
    completed_by_date = {
        row.activity_date: row.sessions_count for row in completed_result.fetchall()
    }
    abandoned_by_date = {
        row.activity_date: row.abandoned_count for row in abandoned_result.fetchall()
    }
    notes_by_date = {
        row.activity_date: row.notes_count for row in notes_result.fetchall()
    }
...
async def get_dashboard_data(
    ...
    """
    logger.info(
        f"Fetching dashboard data for user {user_id} with period {period} and timezone {user_timezone}"
    )
