...
class NoteBase(BaseModel):
    ...
    title: Optional[str] = Field(
        default=None, max_length=255, description="Optional title for the note"
    )
    tags: List[str] = Field(
        default_factory=list, description="List of tags for the note"
    )
...
class NoteCreate(NoteBase):
    ...
    learning_project_id: Optional[UUID] = Field(
        default=None,
        description="Optional learning project ID to associate the note with",
    )
...
class NoteUpdate(BaseModel):
    ...
    tags: Optional[List[str]] = Field(
        default=None, description="List of tags for the note"
    )
...
class NoteDetailResponse(NoteResponse):
    ...
    learning_project_name: Optional[str] = Field(
        default=None, description="Name of the associated learning project"
    )
    similarity_score: Optional[float] = Field(
        default=None,
        description="Similarity score for semantic search results (0.0-1.0, higher is more similar)",
    )
