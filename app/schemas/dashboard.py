...
class DailyActivityResponse(BaseModel):
    ...
    sessions_count: int = Field(
        description="Number of completed sessions on this local date"
    )
    abandoned_sessions_count: int = Field(
        description="Number of abandoned sessions on this local date"
    )
...
class SessionTimeResponse(BaseModel):
    ...
    duration: Optional[int] = Field(
        description="Actual duration in minutes", default=None
    )
    project_name: Optional[str] = Field(
        description="Name of the learning project", default=None
    )
...
class FocusHeatmapResponse(BaseModel):
    ...
    cells: List[FocusHeatmapCell] = Field(
        description="Heatmap cells with aggregated focus data"
    )
    max_minutes: int = Field(
        description="Maximum minutes in any cell (for normalization)", ge=0
    )
...
class DashboardResponse(BaseModel):
    ...
    project_stats: List[ProjectStatsResponse] = Field(
        description="Statistics by project"
    )
    daily_activity: List[DailyActivityResponse] = Field(
        description="Daily activity chart data (timezone-aware)"
    )
    session_times: List[SessionTimeResponse] = Field(
        description="Session times chart data"
    )
    focus_heatmap: FocusHeatmapResponse = Field(
        description="Focus heatmap aggregated by day-of-week and hour"
    )
